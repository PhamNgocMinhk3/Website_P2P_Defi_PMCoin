...
type P2PTab = 'marketplace' | 'create' | 'my-orders' | 'history';
...
export class P2PComponent implements OnInit, OnDestroy {
  activeTab: 'marketplace' | 'create' | 'my-orders' | 'history' = 'marketplace';
  ...
  // ADDED: Withdrawal specific properties
  withdrawPayload = {
    amount: null as number | null,
    bankName: 'BIDV',
    accountNumber: '',
    accountName: '' // This should be pre-filled and read-only
  };
  isProcessingWithdraw = false;
  isProcessingDeposit = false;

  // ADDED: List of available banks for the withdrawal form
  availableBanks: string[] = [
    'BIDV', 'Vietcombank', 'Techcombank', 'ACB', 'MB Bank', 
    'VietinBank', 'VPBank', 'Sacombank', 'Agribank', 'TPBank'
  ];
  ...
  quickSellDebounceTimer: any;
  // --- END QUICK SELL MODAL STATE ---
  ...
  setActiveTab(tab: 'marketplace' | 'create' | 'my-orders' | 'history'): void {
    this.activeTab = tab;
  ...
  // FIX: Workaround for strict template type checking
  isTabActive(tabName: 'marketplace' | 'create' | 'my-orders' | 'history'): boolean {
  ...
  handleDeposit(): void {
    ...
    if (!this.depositAmount || this.depositAmount <= 0) {
      this.notificationService.show('Vui lòng nhập số tiền hợp lệ.', 'error');
      return;
    }

    if (this.depositAmount < MIN_AMOUNT || this.depositAmount > MAX_AMOUNT) {
      this.notificationService.show(
        `Số tiền nạp phải từ ${MIN_AMOUNT.toLocaleString('vi-VN')}đ đến ${MAX_AMOUNT.toLocaleString('vi-VN')}đ.`,
        'error'
      );
      return;
    }
    ...
    this.notificationService.show('Đang tạo yêu cầu nạp tiền...', 'info');

    this.paymentService.createVnpayUrl({ 
      amount: Math.floor(this.depositAmount) // Ensure whole number
    }).subscribe({
      next: (response) => {
        this.notificationService.show('Đang chuyển hướng đến cổng thanh toán...', 'success');
        window.location.href = response.paymentUrl; // Redirect to VNPay
      },
      error: (err) => {
        this.notificationService.show(`Lỗi: ${err.error?.message || 'Không thể tạo yêu cầu nạp tiền.'}`, 'error');
  ...
  openWithdrawModal(): void {
    // FIX: Implement actual modal opening logic and fetch user profile for account name
    if (!this.authService.isAuthenticated) {
      this.notificationService.show('Vui lòng đăng nhập để rút tiền.', 'warning');
      return;
    }
    this.userProfileService.getProfile().subscribe({
      next: (profileResponse) => {
        if (profileResponse.success && profileResponse.data) {
          // FIX: Construct fullName from firstName and lastName
          this.withdrawPayload.accountName = `${profileResponse.data.firstName} ${profileResponse.data.lastName}`.toUpperCase();
          this.isWithdrawModalVisible = true;
        } else {
          this.notificationService.show('Không thể lấy thông tin người dùng. Vui lòng thử lại.', 'error');
        }
      },
      error: (err) => {
        this.notificationService.show(err.error?.message || 'Lỗi khi tải thông tin người dùng.', 'error');
      }
    });
  }
  ...
  closeWithdrawModal(): void {
    ...
    // Reset form, but keep accountName if it was fetched
    this.withdrawPayload = {
      amount: null,
      bankName: 'BIDV',
      accountNumber: '',
      accountName: this.withdrawPayload.accountName // Keep the fetched name
    };
  }

  async handleWithdraw(): Promise<void> {
    const { amount, bankName, accountNumber, accountName } = this.withdrawPayload;

    if (!amount || amount < 100000) { // Min withdrawal 100,000 VND
      this.notificationService.show('Vui lòng nhập số tiền hợp lệ (tối thiểu 100,000đ).', 'error');
      return;
    }
    if (!accountNumber.trim()) {
      this.notificationService.show('Vui lòng nhập số tài khoản ngân hàng.', 'error');
      return;
    }
    if (!accountName.trim()) {
      this.notificationService.show('Không thể xác định tên chủ tài khoản. Vui lòng liên hệ hỗ trợ.', 'error');
      return;
    }

    this.isProcessingWithdraw = true;
    try {
      // Step 1: Lock tokens on-chain by calling the smart contract
      this.notificationService.show('Đang yêu cầu khóa token trên ví của bạn...', 'info');
      // The `requestWithdrawalFromTreasury` method needs to be implemented in Web3P2PService
      // It will require the user to approve the treasury contract to spend their VNDT,
      // and then call the `requestWithdrawal` function on the VNDTreasury contract.
      const txHash = await this.web3P2PService.requestWithdrawalFromTreasury(amount.toString());
      this.notificationService.show(`Khóa token thành công! Tx: ${txHash.slice(0,10)}...`, 'success');

      // Step 2: Send the request to the backend for admin processing
      const payloadWithTx = { ...this.withdrawPayload, transactionHash: txHash };
      
      this.paymentService.requestWithdrawal(payloadWithTx).subscribe({
        next: (res: any) => {
          this.notificationService.show('Yêu cầu rút tiền đã được gửi. Vui lòng chờ admin xử lý.', 'success');
          this.closeWithdrawModal();
        },
        error: (err: any) => {
          this.notificationService.show(err.error?.message || 'Gửi yêu cầu lên server thất bại. Vui lòng liên hệ hỗ trợ.', 'error');
          // CRITICAL: If backend fails, we need to refund the locked tokens on-chain.
          // This requires a new method in Web3P2PService and a corresponding function in VNDTreasury contract.
          // For now, we just log and notify.
          console.error("Backend failed to process withdrawal request after tokens were locked on-chain. User might need manual refund.", err);
          this.notificationService.show("Lỗi nghiêm trọng: Yêu cầu rút tiền thất bại sau khi token đã bị khóa. Vui lòng liên hệ hỗ trợ để được hoàn tiền.", "error");
        }
      });

    } catch (error: any) {
      this.notificationService.show(`Lỗi khi xử lý rút tiền: ${error.reason || error.message}`, 'error');
    } finally {
  ...
  openQuickSellModal(): void {
    if (!this.walletInfo) {
      this.notificationService.show('Vui lòng kết nối ví để sử dụng tính năng này.', 'warning');
      return;
    }
    this.isQuickSellModalVisible = true;
    this.resetQuickSellForm();
    this.fetchQuickSellTokenBalance();
    this.fetchQuickSellRates();
  }

  closeQuickSellModal(): void {
    this.isQuickSellModalVisible = false;
  }

  resetQuickSellForm(): void {
    this.quickSellState.sellToken = 'BTC';
    this.quickSellState.sellAmount = null;
    this.quickSellState.vndReceived = 0;
    this.quickSellState.isProcessing = false;
    this.quickSellState.balance = 0;
  }

  fetchQuickSellRates(): void {
    this.http.get<any>('/api/quicksell/rates').subscribe({
      next: (res) => {
        if (res.success) {
          this.quickSellState.rates = res.data;
          this.calculateQuickSellVNDT(); // Recalculate in case amount was already entered
        } else {
          this.notificationService.show('Không thể tải tỷ giá Bán Nhanh.', 'error');
        }
      },
      error: (err) => {
        this.notificationService.show('Lỗi máy chủ khi tải tỷ giá.', 'error');
      }
    });
  }

  fetchQuickSellTokenBalance(): void {
    if (!this.walletInfo) return;
    const tokenToFetch = this.quickSellState.sellToken;
    this.web3P2PService.getTokenBalance(tokenToFetch, this.walletInfo.address)
      .then(balance => {
        this.quickSellState.balance = parseFloat(balance);
      })
      .catch(() => {
        this.quickSellState.balance = 0;
      });
  ...
  onQuickSellTokenChange(): void {
    this.fetchQuickSellTokenBalance();
    this.calculateQuickSellVNDT();
  }

  onQuickSellAmountChange(): void {
    if (this.quickSellDebounceTimer) clearTimeout(this.quickSellDebounceTimer);
    this.quickSellDebounceTimer = setTimeout(() => {
      this.calculateQuickSellVNDT();
    }, 300); // 300ms debounce
  }

  calculateQuickSellVNDT(): void {
    const { sellToken, sellAmount } = this.quickSellState;
    if (sellAmount && sellAmount > 0) {
      // FIX: Use the p2pService to calculate the price against a stablecoin (e.g., USDT, represented as VND in your config)
      // The backend now handles the conversion correctly. We just display the result.
      this.p2pService.calculateExchange(sellToken, 'VND', sellAmount).subscribe({
        next: (res) => {
          if (res.success && res.data) {
            // The backend now returns the final VNDT amount directly in buyAmount.
            this.quickSellState.vndReceived = res.data.buyAmount;
          }
        }
      });
    } else {
  ...
  getQuickSellableTokens(): string[] {
    // Exclude VNDT from the list of tokens you can sell
    return this.availableTokens.filter(t => t !== 'VNDT' && t !== 'VND');
  }

  setMaxSellAmount(): void {
    this.quickSellState.sellAmount = this.quickSellState.balance;
    this.onQuickSellAmountChange();
  }

  async handleQuickSell(): Promise<void> {
    const { sellToken, sellAmount } = this.quickSellState;

    if (!sellAmount || sellAmount <= 0) {
      this.notificationService.show('Vui lòng nhập số lượng hợp lệ.', 'error');
      return;
    }

    this.quickSellState.isProcessing = true;
    try {
      // 1. Check for approval
      // FIX: The spender for a QuickSell transaction is the QuickSell contract itself.
      this.notificationService.show(`Kiểm tra quyền chi tiêu cho QuickSell Contract...`, 'info');
      const needsApprove = await this.web3P2PService.checkTokenApproval(
        sellToken,
        this.walletInfo!.address,
        sellAmount.toString(),
        'quicksell' // Specify the spender context
      );

      // 2. If needed, approve
      if (needsApprove) {
        this.notificationService.show(`Đang yêu cầu ký để duyệt chi ${sellToken}...`, 'info');
        await this.web3P2PService.approveQuickSell(sellToken, sellAmount.toString());
        this.notificationService.show('Duyệt chi thành công!', 'success');
      }

      // 3. Execute the quick sell
      this.notificationService.show(`Đang thực hiện bán ${sellAmount} ${sellToken}...`, 'info');
      const txHash = await this.web3P2PService.quickSellToken(sellToken, sellAmount.toString());
      this.notificationService.show(`Bán nhanh thành công! Tx: ${txHash.slice(0, 10)}...`, 'success');

      this.closeQuickSellModal();
    } catch (error: any) {
      const errorMessage = error.reason || error.message || 'Giao dịch đã bị từ chối hoặc thất bại.';
      this.notificationService.show(`Lỗi: ${errorMessage}`, 'error');
    } finally {
      this.quickSellState.isProcessing = false;
    }
  }

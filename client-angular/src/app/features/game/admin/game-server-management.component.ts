...
export class GameServerManagementComponent implements OnInit, OnDestroy {
  ...
  async depositToTreasury(): Promise<void> {
    ...
    try {
      // FIX: The deposit logic should be client-side, not via backend API,
      // because the smart contract function `depositForTreasury` is `onlyOwner`.
      // The process is: 1. Approve, 2. Deposit.

      this.notificationService.show('Requesting approval to spend PM tokens...', 'info');
      const approveTxHash = await this.smartContractService.approvePMTokens(depositAmount);
      this.notificationService.show(`Approval successful! Tx: ${approveTxHash.slice(0, 10)}... Now depositing.`, 'success');

      // The actual deposit function on the contract is `depositForTreasury`.
      // The smart contract service should have a method that calls this.
      // Let's assume `depositPMToTreasury` is the correct client-side method.
      // We will need to create it if it doesn't exist.
      const depositTxHash = await this.smartContractService.depositPMToTreasury(depositAmount);
      this.notificationService.show(`Deposit successful! Tx: ${depositTxHash.slice(0, 10)}...`, 'success');

      this.serverStats.treasuryOperations.depositAmount = ''; // Clear input
      await this.refreshData();
    } catch (error: any) {
      // The error now comes directly from MetaMask/ethers, providing more detail.
      const errorMessage = error.reason || error.data?.message || error.message || 'An unknown error occurred.';

...
import { VND_TREASURY_ABI } from '../contracts/abi-vnd-treasury'; // ADDED
import { QUICKSELL_ABI } from '../contracts/abi-quicksell'; // ADDED
import { QUICKSELL_ADDRESS } from '../contracts/contract-address-quicksell'; // ADDED
...
export class Web3P2PService implements OnDestroy {
  ...
  private quickSellContractAddress: string = QUICKSELL_ADDRESS; // ADDED
  private quickSellContractABI: any[] = QUICKSELL_ABI; // ADDED

  private tokenAddresses = {
    // Note: VNDT is now named VND to match the create order form.
    'BTC': '0xD0D23e174DE928Ff8D0E08B4437EB74B0aF93563',
    'ETH': '0x8A974476a492231Fe57d894Ff5AF5EedcA925498',
    'VND': environment.VND_TOKEN_ADDRESS, // FIX: Use address from environment
  ...
  private tokenDecimals: Record<string, number> = {
    ...
    'ETH': 6,
    'VND': 6,
    'VNDT': 6, // Keep VNDT for withdrawal logic
    'PM': 6
  };
  ...
  private quickSellContractInstance: ethers.Contract | null = null; // ADDED
  ...
  private initializeContract(): void {
    if (this.provider) {
      try {
        ...
        // ADDED: Initialize QuickSell contract instance
        this.quickSellContractInstance = new ethers.Contract(
          this.quickSellContractAddress,
          this.quickSellContractABI,
          this.provider
        );
        console.log('⚡ QuickSell contract initialized:', this.quickSellContractAddress);
  ...
  async checkTokenApproval(
    ...
    userAddress: string, 
    amount: string,
    spenderContext: 'p2p' | 'quicksell' = 'p2p' // ADDED: spenderContext parameter
  ): Promise<boolean> {
    try {
      if (!this.provider) {
      ...
      // Determine which contract address is the spender
      const spenderAddress = spenderContext === 'quicksell' ? this.quickSellContractAddress : this.contractAddress;
  ...
  // ADDED: Implementation for requesting a withdrawal
  async requestWithdrawalFromTreasury(amount: string): Promise<string> {
    if (!this.signer) {
      throw new Error('Wallet not connected. Please connect your wallet to request a withdrawal.');
    }

    const vndTokenAddress = environment.VND_TOKEN_ADDRESS;
    const vndTreasuryAddress = environment.VND_TREASURY_ADDRESS;

    if (!vndTokenAddress || !vndTreasuryAddress) {
      throw new Error('VNDT or Treasury contract address is not configured in the environment.');
    }
    ...
    const tokenContract = new ethers.Contract(vndTokenAddress, erc20ABI, this.signer);
    const treasuryContract = new ethers.Contract(vndTreasuryAddress, VND_TREASURY_ABI, this.signer);

    const amountWei = ethers.parseUnits(amount, this.tokenDecimals['VNDT']);
    const userAddress = await this.signer.getAddress();

    // Step 1: Check current allowance
    const currentAllowance = await tokenContract['allowance'](userAddress, vndTreasuryAddress);

    // Step 2: If allowance is not enough, approve
    if (currentAllowance < amountWei) {
      const approveTx = await tokenContract['approve'](vndTreasuryAddress, amountWei);
      await approveTx.wait(); // Wait for the approval transaction to be mined
    }

    // Step 3: Call requestWithdrawal on the treasury contract
    const requestTx = await treasuryContract['requestWithdrawal'](vndTokenAddress, amountWei);
    const receipt = await requestTx.wait();

    if (receipt.status === 0) {
      throw new Error('On-chain transaction to lock tokens for withdrawal failed.');
    }
  ...
  /**
   * Approves the QuickSell contract to spend a user's tokens.
   * @param tokenSymbol The symbol of the token to approve (e.g., 'BTC').
   * @param amount The amount to approve.
   * @returns The transaction hash of the approval.
   */
  async approveQuickSell(tokenSymbol: string, amount: string): Promise<string> {
    if (!this.signer) {
      throw new Error('Wallet not connected');
    }
    const tokenAddress = this.tokenAddresses[tokenSymbol as keyof typeof this.tokenAddresses];
    ...
    const tx = await tokenContract['approve'](this.quickSellContractAddress, amountWei);
    await tx.wait();
    return tx.hash;
  }

  /**
   * Sells a token for VNDT using the QuickSell contract.
   * @param tokenSymbol The symbol of the token to sell.
   * @param amount The amount to sell.
   * @returns The transaction hash.
   */
  async quickSellToken(tokenSymbol: string, amount: string): Promise<string> {
    if (!this.signer || !this.quickSellContractInstance) {
      throw new Error('Wallet or QuickSell contract not ready');
    }

    const tokenToSellAddress = this.tokenAddresses[tokenSymbol as keyof typeof this.tokenAddresses];
    if (!tokenToSellAddress) {
      throw new Error(`Token not supported for QuickSell: ${tokenSymbol}`);
    }

    const decimals = this.tokenDecimals[tokenSymbol] ?? 6;
    const amountToSellWei = ethers.parseUnits(amount, decimals);

    // We set minVNDTToReceive to 0 for simplicity, as QuickSell rates are fixed by admin.
    // In a real-world scenario with fluctuating prices, this would be a calculated value with slippage.
    const minVNDTToReceive = 0;

    const contractWithSigner = this.quickSellContractInstance.connect(this.signer);
    // FIX: Cast the contract instance to 'any' to bypass TypeScript's strict type checking for dynamic method calls.
    // This resolves the "Property 'sellTokenForVNDT' does not exist on type 'BaseContract'" error.
    const tx = await (contractWithSigner as any).sellTokenForVNDT(tokenToSellAddress, amountToSellWei, minVNDTToReceive);
    await tx.wait();

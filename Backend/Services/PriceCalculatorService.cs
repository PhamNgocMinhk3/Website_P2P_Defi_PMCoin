        ...
        public async Task<decimal> GetTokenPriceAsync(string token)
        {
            try
            {
                ...
                return token switch
                {
                    ...
                    // FIX: The price of 1 VND in USD. This is the correct way to handle the conversion.
                    // If 1 USD = 25,545 VND, then 1 VND = 1/25545 USD.
                    // The calculation `sellTokenPrice / buyTokenPrice` will then correctly become `sellTokenPrice * 25545`.
                    SupportedTokens.VND => 1.0m / _configuration.GetValue<decimal>("AppSettings:UsdToVndtRate", 25545),

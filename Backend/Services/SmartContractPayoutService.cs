...
namespace TradeFinanceBackend.Services
{
    ...
    public class SmartContractPayoutService : ISmartContractPayoutService
    {
        ...
        {
            // STEP 1: Check if the result is a loss. If so, skip the payout process.
            // This is the correct and final logic.
            if (result == "LOSE")
            {
                _logger.LogInformation("Payout skipped for a LOSE result. Bet ID: {BetId}", bet.Id);
                return "SKIPPED_LOSE"; // Return a special string for logging purposes.
            }
            ...
            if (!bet.ContractBetId.HasValue || bet.ContractBetId.Value <= 0)
            {
                ...
                // Do not fallback, as this is a system error that needs to be fixed at the source. Return null to indicate failure.
                return null;
            }
            ...
            try
            {
                ...
                if (contractBalance < payoutAmount)
                {
                    ...
                    await _signalRService.SendNotificationToUser(bet.UserAddress, "Lỗi: Máy game không đủ tiền trả thưởng. Vui lòng liên hệ admin.", "error"); // Notify user
            ...
            // STEP 4: Main Logic - Call the Smart Contract to process the payout.
            // All complex and faulty "fallback" logic has been removed.
            try
            {

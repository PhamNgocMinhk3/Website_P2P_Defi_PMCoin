...
// ADDED: DTO for refundWithdrawal function
[Function("refundWithdrawal")]
public class RefundWithdrawalFunction : FunctionMessage
{
    [Parameter("address", "tokenAddress", 1)] public string TokenAddress { get; set; } = string.Empty;
    [Parameter("address", "to", 2)] public string To { get; set; } = string.Empty;
    [Parameter("uint256", "amount", 3)] public BigInteger Amount { get; set; }
}

// ADDED: DTO for claimWithdrawal function
[Function("claimWithdrawal")]
public class ClaimWithdrawalFunction : FunctionMessage
{
    [Parameter("address", "tokenAddress", 1)] public string TokenAddress { get; set; } = string.Empty;
...
namespace TradeFinanceBackend.Services
{
    ...
    public class ManualPayoutFunction : FunctionMessage
    {
        ...
        // DTO for the transferFromTreasuryToUser function in the VNDTreasury contract
    [Function("transferFromTreasuryToUser")]
    public class TransferFromTreasuryToUserFunction : FunctionMessage
    {
    ...
    public class SmartContractService : ISmartContractService
    {
        ...
        private readonly string _vndTokenAddress;
        private readonly string _vndTokenAbi;
        ...
        {
            var blockchainConfig = configuration.GetSection("Blockchain:CoreChain");
            _vndTokenAddress = blockchainConfig["VNDTokenAddress"] ?? throw new InvalidOperationException("VNDTokenAddress is missing in configuration.");
            _vndTokenAbi = File.ReadAllText(blockchainConfig["VNDTokenAbiPath"] ?? "abi_VND.json");
        ...
        public async Task<ContractStatsDto> GetContractStats()
        {
            try
            {
                ...
                // Using "pending" ensures we see the result of our transactions immediately.
                var pendingBlock = new Nethereum.RPC.Eth.DTOs.BlockParameter();

                var balanceInWeiTask = balanceHandler.QueryAsync<BigInteger>(contractAddress!, new GetTreasuryBalanceFunction(), pendingBlock);
                var gameStatsTask = gameStatsHandler.QueryDeserializingToObjectAsync<GetGameStatsOutputDTO>(new GetGameStatsFunction(), contractAddress!, pendingBlock);
        ...
        public async Task<string> TransferVndtFromTreasuryAsync(string userAddress, decimal amount)
        {
            // CRITICAL FIX: The logic was incorrectly transferring from the backend's private wallet.
            // This has been corrected to call the `transferFromTreasuryToUser` function on the dedicated `VNDTreasury` contract.
            // This ensures that funds deposited into the treasury are correctly used for user on-ramps.
            try
            {
                var web3 = GetWeb3ClientWithAccount();
                var blockchainConfig = _configuration.GetSection("Blockchain:CoreChain");
                var vndTreasuryAddress = blockchainConfig["VNDTreasuryAddress"] ?? throw new InvalidOperationException("VNDTreasuryAddress is missing in configuration.");
                var vndTokenAddress = blockchainConfig["VNDTokenAddress"] ?? throw new InvalidOperationException("VNDTokenAddress is missing in configuration.");

                // The backend wallet pays for the gas to execute the transaction on the treasury contract.
                var backendWalletAddress = web3.TransactionManager.Account.Address;

                _logger.LogInformation("Initiating transfer from VNDTreasury contract. Treasury: {TreasuryAddress}, Token: {TokenAddress}, Recipient: {Recipient}, Amount: {Amount}",
                    vndTreasuryAddress, vndTokenAddress, userAddress, amount);

                // Assume 6 decimals for VNDT as per standard configuration.
                const int decimals = 6;

                // CRITICAL FIX: Do not use Web3.Convert.ToWei(decimal, int) as it can produce incorrect results for whole numbers.
                // Manually calculate the value in the smallest unit to ensure correctness.
                // Example: 50000.00m * 10^6 = 50000000000.
                var multiplier = BigInteger.Pow(10, decimals);
                var amountInSmallestUnit = new BigInteger(amount * (decimal)multiplier);
                var transferFunction = new TransferFromTreasuryToUserFunction
                {
                    TokenAddress = vndTokenAddress,
                    To = userAddress,
                    Amount = amountInSmallestUnit
                };

                var handler = web3.Eth.GetContractTransactionHandler<TransferFromTreasuryToUserFunction>();
                var receipt = await handler.SendRequestAndWaitForReceiptAsync(vndTreasuryAddress, transferFunction);

                if (receipt?.Status?.Value != 1)
                {
                    _logger.LogError("VNDT transfer from treasury failed. TxHash: {TxHash}, Status: {Status}", receipt?.TransactionHash, receipt?.Status?.Value);
                    throw new Exception($"Smart contract transaction failed to transfer from treasury. Status: {receipt?.Status?.Value}");
                }

                _logger.LogInformation("✅ VNDT transfer successful. TxHash: {TxHash}", receipt.TransactionHash);
                return receipt.TransactionHash;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error transferring VNDT to {UserAddress}", userAddress);
                throw;
            }
        }

        public async Task<string> RefundWithdrawalAsync(string userAddress, decimal amount)
        {
            try
            {
                var web3 = GetWeb3ClientWithAccount();
                ...
                _logger.LogInformation("Initiating withdrawal refund to {UserAddress} for amount {Amount}", userAddress, amount);

                const int decimals = 6;
                var multiplier = BigInteger.Pow(10, decimals);
                var amountInSmallestUnit = new BigInteger(amount * (decimal)multiplier);

                var refundFunction = new RefundWithdrawalFunction
                {
                    TokenAddress = vndTokenAddress,
                    To = userAddress,
                    Amount = amountInSmallestUnit
                ...
                var handler = web3.Eth.GetContractTransactionHandler<RefundWithdrawalFunction>();
                var receipt = await handler.SendRequestAndWaitForReceiptAsync(vndTreasuryAddress, refundFunction);

                if (receipt?.Status?.Value != 1)
                {
                    throw new Exception($"Smart contract transaction failed to refund withdrawal. Status: {receipt?.Status?.Value}");
                }
            ...
            {
                _logger.LogError(ex, "Error processing withdrawal refund for user {UserAddress}", userAddress);
                throw;
            }
        }

        public async Task<string> ClaimWithdrawalAsync(decimal amount)
        {
            try
            {
                var web3 = GetWeb3ClientWithAccount();
                ...
                _logger.LogInformation("Initiating withdrawal claim from Treasury contract for amount {Amount}", amount);

                const int decimals = 6;
                var multiplier = BigInteger.Pow(10, decimals);
                var amountInSmallestUnit = new BigInteger(amount * (decimal)multiplier);

                var claimFunction = new ClaimWithdrawalFunction
                {
                    TokenAddress = vndTokenAddress,
                    Amount = amountInSmallestUnit
                };

                var handler = web3.Eth.GetContractTransactionHandler<ClaimWithdrawalFunction>();
                var receipt = await handler.SendRequestAndWaitForReceiptAsync(vndTreasuryAddress, claimFunction);

                if (receipt?.Status?.Value != 1)
                {
                    throw new Exception($"Smart contract transaction failed to claim withdrawal. Status: {receipt?.Status?.Value}");
                }

                _logger.LogInformation("✅ Successfully claimed {Amount} VNDT from Treasury. TxHash: {TxHash}", amount, receipt.TransactionHash);
                return receipt.TransactionHash;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error processing withdrawal claim for amount {Amount}", amount);
                throw;
            }
        }
    }

...
// FIX: Configure JSON serialization to handle circular references.
// This prevents crashes when serializing related EF Core entities (e.g., FiatTransaction -> User -> FiatTransaction).
// The error ERR_INCOMPLETE_CHUNKED_ENCODING in the browser is a symptom of the backend crashing during serialization.
...
// CRITICAL FIX: Temporarily remove the global cookie policy.
// The existing policy in Program.cs was conflicting with the more specific,
// environment-aware cookie settings in AuthController.cs, causing the session
// cookie to be rejected by the browser after the VNPay redirect.
// By removing this, we give full control to AuthController.
...
builder.Services.AddScoped<IQuickSellService, QuickSellService>();
...
// Add HttpClientFactory for general use, like in VnpayService
...
builder.Services.AddHostedService<QuickSellRateUpdateService>(); // ADDED: Auto-update QuickSell rates
